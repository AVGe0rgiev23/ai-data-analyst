...
export function buildSystemPrompt(source: SourceWithSchema): string {
  const columnLines = source.columns.map((column) => {
    ...
    const description = column.description
      ...
      : '';
    // Tells the agent why a date-looking column is text, so it does not spend a
    // tool call on date_trunc against a VARCHAR and then retry. Sanitised like
    // everything else: this string is generated by the profiler today, but it
    // arrives here from the database, and nothing from storage is trusted
    // simply because the application wrote it.
    const dateNote = column.dateWarning
      ? ` [text, not a date: ${sanitizeForPrompt(column.dateWarning, 100)}]`
      : '';
    return `  - ${name} (${stats})${dateNote}${description}`;
  });
...
Table \`${sanitizeForPrompt(source.tableName)}\` (${source.rowCount.toLocaleString()} rows${
    source.duplicateRows && source.duplicateRows > 0
      ? `, of which ${source.duplicateRows.toLocaleString()} are exact duplicates of an earlier row`
      : ''
  }, loaded from "${sanitizeForPrompt(source.name)}"):
${columnLines.join('\n')}
